(() => {
  ...
  // Daily gift config
  const DAILY_GIFT_AMOUNT = 150; // free gold per day
  ...
  // ======= State =======
  let level = 1;
  let rng = mulberry32(Date.now() % 2**31);
  let kidMode = loadKidMode();
  applyTheme(loadTheme());
  let muted = loadMuted();
  updateMuteButton();

  // Gold & stats & inventory
  let gold = loadGold();
  ...
  const COYOTE_TIME = ()=> kidMode ? 0.12 : 0.08;
  const JUMP_BUFFER = ()=> kidMode ? 0.18 : 0.12;
  ...
  let camY = 0;
  /** @type {Platform[]} */ let platforms = [];
  let goalY = 0;
  let currentLevelHeight = 0;

  const input = { left:false, right:false, jump:false, jumpConsumed:false };

  let levelTime = 0;   // seconds for active level
  let totalTime = 0;   // seconds of completed levels
   ...
   *  @property {number} w
   *  @property {number} h
   *  @property {"static"|"moving"} type
   *  @property {number} [vx]
   *  @property {number} [range]
   *  @property {number} phase
   *  @property {number} [_origX]
   */
  ...
  window.addEventListener('keydown', (e)=>{
    ...
    if (e.code === 'Escape'){
      if (gameState==='running') pauseGame(); else if (gameState==='paused') resumeWithCountdown(2);
      e.preventDefault();
      return;
    }
    e.preventDefault(); input[k] = true; if (k==='jump') input.jumpConsumed=false;
    ensureAudio();
  }, {passive:false});
  ...
  btnContinue.addEventListener('click', ()=>{
    ...
    if (save){
      ...
      updateGoldUI();
      startLevel(save.level||1, false, false);
      startCountdown(2); ensureAudio();
    }
  });
  ...
  // Daily gift
  giftBtn.addEventListener('click', ()=>{ tryClaimDailyGift(); });
  refreshGiftButton();
  ...
  function update(dt){
    // Timers
    levelTime += dt;
    elLevelTime.textContent = formatTime(levelTime);
    ...
    // Collide (landing from above)
    let groundedThisFrame = false;
    if (player.vy >= 0){
      for (const p of platforms){
        if (player.y + player.h <= p.y && nextY + player.h >= p.y){
          const px1 = p.x, px2 = p.x + p.w;
          const plx1 = nextX, plx2 = nextX + player.w;
          if (plx2 > px1 && plx1 < px2){
            nextY = p.y - player.h;
            player.vy = 0;
    ...
    // finalize combo climb when touching ground
    ...
    // Gold for new upward progress (climb)
    ...
    if (player.y < bestY){
      ...
      if (climbGold>0){
        addGold(climbGold); // counts for level + lifetime
        levelProgressBestY[level] = player.y;
        comboClimbThisAir += climbGold; // accumulate until next landing
      }
    }

    // Win
    if (player.y <= goalY){
      // finalize combo for the run (if ended mid-air at goal)
      if (comboClimbThisAir > longestComboClimb){ longestComboClimb = comboClimbThisAir; saveLongestComboClimb(longestComboClimb); }
      if (comboClimbThisAir > bestComboThisLevel){ bestComboThisLevel = comboClimbThisAir; }
      comboClimbThisAir = 0;
      ...
      // PB per level
      const key = STORE.pb(level); const prev = parseFloat(localStorage.getItem(key)); const pbImproved = isFinite(prev) ? (levelTime < prev) : true; if (pbImproved) localStorage.setItem(key, String(levelTime));
      elPB.textContent = `PB: ${formatTime(parseFloat(localStorage.getItem(key)) || levelTime)}`;

      // Update best level reached
      if (level > bestLevel){ bestLevel = level; saveBestLevel(bestLevel); }

      // Update lifetime stats at end of level
      ...
      // Show run summary (do NOT auto-start next level)
      openSummary({
        level,
        time: levelTime,
        isPB: pbImproved,
        ...
        timeBonus,
        levelBonus,
        combo: bestComboThisLevel,
        streak: streakNoDeath,
        bestStreak: bestNoDeathStreak
      });
      return;
    }
    ...
    if (player.y > fallLimit){
      deaths++; saveDeaths(deaths);
      streakNoDeath = 0; // reset streak on fall
      flashStatus(kidMode? `${kidEncouragement()} Restarting…`:'Fell! Restarting level…');
      comboClimbThisAir = 0; bestComboThisLevel = 0; // reset combo tracking for this level
  ...
  function drawPlatform(ctx, p){
    const y=p.y, x=p.x, w=p.w, h=p.h; ctx.fillStyle='#242325'; ctx.strokeStyle='rgba(255,255,255,0.05)'; ctx.lineWidth=1.2; roundRect(ctx,x,y,w,h,6); ctx.fill(); ctx.stroke();
    ...
    ctx.fillStyle='rgba(255,255,255,0.06)'; for (let i=0;i<Math.max(2, Math.floor(w/100));i++){ const bx = x + 12 + i*(w-24)/Math.max(1,(Math.floor(w/100))); ctx.beginPath(); ctx.arc(bx, y + h - 8, 2, 0, Math.PI*2); ctx.fill(); }
  }
  ...
  function openSummary({ level, time, isPB, gold, timeBonus, levelBonus, combo, streak, bestStreak }){
    gameState = 'overlay';
    sumTitle.textContent = `Level ${level} Complete!`;
    sumSub.textContent = `Great job!`;
    sumTime.textContent = formatTime(time);
    sumPB.hidden = !isPB;
    sumGold.textContent = String(gold);
    sumTimeBonus.textContent = String(timeBonus);
    sumLevelBonus.textContent = String(levelBonus);
    sumCombo.textContent = String(combo);
    sumStreak.textContent = String(streak);
    sumBestStreak.textContent = `Best: ${bestStreak}`;
  ...
  function startLevel(lvl, isRestart=false, showOverlayAtStart=false){
    level = lvl; elLevel.textContent = `Level ${level}`; rng = mulberry32((Date.now() + level*1337) % 2**31);
    player.x = WORLD.w*0.5 - player.w/2; player.y = 0; player.vx=0; player.vy=0; player.onGround=false; player.coyoteTime=0; player.jumpBuffer=0;
    levelTime = 0; elLevelTime.textContent = formatTime(0);
    goldThisLevel = 0; comboClimbThisAir = 0; bestComboThisLevel = 0;
    if (!isRestart) levelProgressBestY[level] = player.y;
    camY = -WORLD.h*0.1;
    ...
    if (kidMode){ gapMin=Math.max(60, gapMin-25); gapMax=Math.max(90, gapMax-35); widthMin=Math.min(170, widthMin+40); widthMax=Math.min(260, widthMax+40); moveChance=Math.max(0.04, moveChance*0.5); moveRangeMin=18; moveRangeMax=44; }

    const height = clamp(1200 + (level-1)*220, 1200, 3600); currentLevelHeight = height; goalY = -height;

    platforms = []; platforms.push({ x: WORLD.w*0.5 - 180, y: 40, w: 360, h: 18, type:'static', phase:0 });
    let lastY = 40 - randRange([40,60]);
    while (lastY > goalY + 80){
      const w = randRange([widthMin, widthMax]);
      const x = clamp(randRange([24, WORLD.w - 24 - w]), 24, WORLD.w - 24 - w);
      const gap = randRange([gapMin, gapMax]);
      const y = lastY - gap;
      if (chance(moveChance)){
        const range = randRange([moveRangeMin, moveRangeMax]);
        platforms.push({ x, y, w, h:16, type:'moving', vx:0, range, _origX:x, phase: randRange([0, Math.PI*2]) });
      } else {
    ...
    const key = STORE.pb(level); const prev = parseFloat(localStorage.getItem(key)); elPB.textContent = `PB: ${isFinite(prev) ? formatTime(prev) : '--:--.--'}`;
  ...
  function buildShop(){
    ...
    SKINS.forEach(skin=>{
      ...
      // Preview setup
      prev.dataset.phase = String(Math.random()*Math.PI*2);
      prev.dataset.skin = skin.id;
      drawSkinPreview(prev, skin, 0);
  ...
  function drawSkinPreview(cnv, skin, t){
    ...
    const phase = parseFloat(cnv.dataset.phase||'0');
    const time = (t||0)/1000 + phase;
    const bob = Math.sin(time*2.0) * 6; // idle bob
  ...
  function startPreviewAnim(){
    function step(t){
      if (!shopOv.classList.contains('show')) return;
      document.querySelectorAll('.skin-preview').forEach((cnv)=>{
        const sid = cnv.dataset.skin; const skin = SKINS.find(s=>s.id===sid) || SKINS[0];
        drawSkinPreview(cnv, skin, t);
      });
  ...
  // ===== Simple SFX with WebAudio (no assets) =====
  let audio = null;

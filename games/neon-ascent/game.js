(() => {
  ...
  function startLevel(lvl, isRestart=false, showOverlayAtStart=false){
    ...
    const WORLD_W=WORLD.w; platforms = [];
    platforms.push({ x: WORLD_W*0.5 - 180, y: 40, w: 360, h: 18, type:'static', phase:0 });
    ...
    if (showOverlayAtStart) {
      // retain climb baseline; only reset on fresh run
    } else {

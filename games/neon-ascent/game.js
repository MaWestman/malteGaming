 ...
 * ----------------------------------------------------- */
(() => {
  console.log('[Neon Ascent] game.js start v7.4'); console.log('[Neon Ascent] BUILD NA-2025-10-07k (skins+fx consolidated for neon-ascent)');
  ...
  , { id:'orbix', name:'Orbix', price:26000, rarity:'epic', unlockLevel:12, body:'#10151e', visor:'#2cf9ff', stripe:'#ff3df2', shape:'orb' }
...
function draw(){
    ...
    drawNeonPipes(ctx);
    ctx.save(); ctx.lineWidth=3; ctx.setLineDash([10,8]); neonStroke(ctx, ()=>{ ctx.beginPath(); ctx.moveTo(24, goalY); ctx.lineTo(WORLD.w-24, goalY); ctx.stroke(); 
  fxDraw(ctx);
}, getAccentColor(), 18); ctx.restore();
  ...
  function applySkinMetrics(skin){ try{
    const s = skin || currentSkin(); const dims = getSkinDims(s);
    const oldW = player.w, oldH = player.h; const feetY = player.y + oldH;
 ...
 player.y = feetY - dims.h;
  ...
  if (shape==='ufo'){
    ...
    const rad=Math.min(w/2,h/2); roundRect(ctx,x,y,w,h,rad); ctx.fill();
    ...
    neonFill(ctx, ()=>{ ctx.beginPath(); ctx.arc(cx, cy, r, 0, Math.PI*2); ctx.fill(); }, getAccent2Color(), 10);
    ...
    neonFill(ctx, ()=>{ ctx.beginPath(); ctx.moveTo(x + w/2, y); ctx.lineTo(x + w, y + h); ctx.lineTo(x, y + h); ctx.closePath(); ctx.fill(); }, getAccent2Color(), 10);

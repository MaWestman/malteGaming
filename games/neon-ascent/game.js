(() => {
  ...
  const STORE = {
    ...
    kid: 'neonAscent_kidMode', theme: 'neonAscent_theme', muted: 'neonAscent_muted', save: 'neonAscent_save',
    gold: 'neonAscent_gold', inv: 'neonAscent_inventory', skin: 'neonAscent_skin', best: 'neonAscent_bestLevel',
    lifetime: 'neonAscent_lifetimeGold', daily: 'neonAscent_dailyGift_lastClaim', levelsCompleted: 'neonAscent_levelsCompleted',
    mostGoldInLevel: 'neonAscent_mostGoldInLevel', deaths: 'neonAscent_deaths', bestNoDeathStreak: 'neonAscent_bestNoDeathStreak',
  ...
  applyTheme(loadTheme()); let muted = loadMuted(); updateMuteButton();
  ...
  window.addEventListener('keydown', (e)=>{ const k = keymap[e.code]; if (!k) return; if (e.code === 'Escape'){ if (gameState==='running') pauseGame(); else if (gameState==='paused') resumeWithCountdown(2); e.preventDefault(); return; } e.preventDefault(); input[k] = true; if (k==='jump') input.jumpConsumed=false; ensureAudio(); }, {passive:false});
  ...
  let last = performance.now(); function loop(now){ const dt = Math.min(0.032, (now - last)/1000); last = now; if (gameState==='countdown'){ countdown -= dt; updateOverlayCountdown(); if (countdown <= 0) setRunning(); } else if (gameState==='running'){ update(dt); } draw(); requestAnimationFrame(loop); } requestAnimationFrame(loop);
  function update(dt){ levelTime += dt; elLevelTime.textContent = formatTime(levelTime); elTotalTime.textContent = formatTime(totalTime);
    ...
    if (player.jumpBuffer > 0) { if (player.onGround || player.coyoteTime > 0){ player.vy = -JUMP_VY; player.onGround = false; player.coyoteTime = 0; player.jumpBuffer = 0; input.jumpConsumed = true; SFX.jump(); } }
    ...
    if (!wasGrounded && player.onGround){ if (comboClimbThisAir > longestComboClimb){ longestComboClimb = comboClimbThisAir; saveLongestComboClimb(longestComboClimb); } if (comboClimbThisAir > bestComboThisLevel){ bestComboThisLevel = comboClimbThisAir; } comboClimbThisAir = 0; addGold(LANDING_GOLD); SFX.land(); }
    for (const p of platforms){ if (p.type==='moving'){ p.phase += dt; const offset = Math.sin(p.phase) * (p.range || 0); p.x = p._origX + offset; if (p.x < 0) p.x = 0; if (p.x + p.w > WORLD.w) p.x = WORLD.w - p.w; } }
    ...
    const bestY = levelProgressBestY[level] ?? player.y; if (player.y < bestY){ const delta = bestY - player.y; const climbGold = Math.floor(delta * CLIMB_GOLD_PER_PX); if (climbGold>0){ addGold(climbGold); levelProgressBestY[level] = player.y; comboClimbThisAir += climbGold; } }
    if (player.y <= goalY){ if (comboClimbThisAir > longestComboClimb){ longestComboClimb = comboClimbThisAir; saveLongestComboClimb(longestComboClimb); } if (comboClimbThisAir > bestComboThisLevel){ bestComboThisLevel = comboClimbThisAir; } comboClimbThisAir = 0; const parSeconds = Math.max(1, currentLevelHeight / PAR_SPEED); const timeBonus = Math.max(0, Math.floor((parSeconds - levelTime) * TIME_BONUS_RATE)); const levelBonus = LEVEL_BASE_BONUS + LEVEL_PER_LEVEL_BONUS * level; const gained = timeBonus + levelBonus; addGold(gained); totalTime += levelTime; const key = STORE.pb(level); const prev = parseFloat(localStorage.getItem(key)); const pbImproved = isFinite(prev) ? (levelTime < prev) : true; if (pbImproved) localStorage.setItem(key, String(levelTime)); elPB.textContent = `PB: ${formatTime(parseFloat(localStorage.getItem(key)) || levelTime)}`; if (level > bestLevel){ bestLevel = level; saveBestLevel(bestLevel); } levelsCompleted++; saveLevelsCompleted(levelsCompleted); if (goldThisLevel > mostGoldInOneLevel){ mostGoldInOneLevel = goldThisLevel; saveMostGoldInLevel(mostGoldInOneLevel); } streakNoDeath++; if (streakNoDeath > bestNoDeathStreak){ bestNoDeathStreak = streakNoDeath; saveBestNoDeathStreak(bestNoDeathStreak); } SFX.goal(); flashStatus(`Level ${level} complete! +${gained}g | ${formatTime(levelTime)}` + (pbImproved?' (PB!)':'')); saveProgress({ level: level+1, gold, totalTime }); openSummary({ level, time: levelTime, isPB: pbImproved, gold: goldThisLevel, timeBonus, levelBonus, combo: bestComboThisLevel, streak: streakNoDeath, bestStreak: bestNoDeathStreak }); return; }
    const fallLimit = camY + WORLD.h + 240; if (player.y > fallLimit){ deaths++; saveDeaths(deaths); streakNoDeath = 0; flashStatus(kidMode? `${kidEncouragement()} Restarting…`:'Fell! Restarting level…'); comboClimbThisAir = 0; bestComboThisLevel = 0; startLevel(level, true, true); return; }
  }
  function draw(){ const w = canvas.width, h = canvas.height; const scaleX = w / WORLD.w, scaleY = h / WORLD.h; const s = Math.min(scaleX, scaleY); const g = ctx.createLinearGradient(0,0,0,h); g.addColorStop(0,'#0b0c0f'); g.addColorStop(1,'#161514'); ctx.fillStyle = g; ctx.fillRect(0,0,w,h); ctx.save(); ctx.translate((w - WORLD.w * s)/2, (h - WORLD.h * s)/2); ctx.scale(s, s); ctx.translate(0, -camY); drawNeonPipes(ctx); ctx.save(); ctx.lineWidth=3; ctx.setLineDash([10,8]); neonStroke(ctx, ()=>{ ctx.beginPath(); ctx.moveTo(24, goalY); ctx.lineTo(WORLD.w-24, goalY); ctx.stroke(); }, getAccentColor(), 18); ctx.restore(); for (const p of platforms) drawPlatform(ctx, p); drawPlayer(ctx, player); ctx.restore(); }
  function drawPlatform(ctx, p){ const y=p.y, x=p.x, w=p.w, h=p.h; ctx.fillStyle='#242325'; ctx.strokeStyle='rgba(255,255,255,0.05)'; ctx.lineWidth=1.2; roundRect(ctx,x,y,w,h,6); ctx.fill(); ctx.stroke(); neonStroke(ctx, ()=>{ ctx.beginPath(); ctx.moveTo(x+3, y+2.5); ctx.lineTo(x+w-3, y+2.5); ctx.stroke(); }, p.type==='moving'? getAccent2Color() : getAccentColor(), p.type==='moving'?22:16); ctx.fillStyle='rgba(255,255,255,0.06)'; for (let i=0;i<Math.max(2, Math.floor(w/100));i++){ const bx = x + 12 + i*(w-24)/Math.max(1,(Math.floor(w/100))); ctx.beginPath(); ctx.arc(bx, y + h - 8, 2, 0, Math.PI*2); ctx.fill(); } }
  function currentSkin(){ return SKINS.find(s=>s.id===equippedSkinId) || SKINS[0]; }
  function drawPlayer(ctx, pl){ const skin = currentSkin(); const x=pl.x, y=pl.y, w=pl.w, h=pl.h; ctx.fillStyle=skin.body; roundRect(ctx,x,y,w,h,10); ctx.fill(); neonFill(ctx, ()=>{ roundRect(ctx, x+8, y+10, w-16, 16, 8); ctx.fillStyle=skin.visor; ctx.fill(); }, skin.visor, 18); neonFill(ctx, ()=>{ roundRect(ctx, x + w*0.15, y + h - 12, w*0.7, 6, 3); ctx.fillStyle=skin.stripe; ctx.fill(); }, skin.stripe, 10); }
  ...
  function openSummary({ level, time, isPB, gold, timeBonus, levelBonus, combo, streak, bestStreak }){ gameState = 'overlay'; sumTitle.textContent = `Level ${level} Complete!`; sumSub.textContent = `Great job!`; sumTime.textContent = formatTime(time); sumPB.hidden = !isPB; sumGold.textContent = String(gold); sumTimeBonus.textContent = String(timeBonus); sumLevelBonus.textContent = String(levelBonus); sumCombo.textContent = String(combo); sumStreak.textContent = String(streak); sumBestStreak.textContent = `Best: ${bestStreak}`; summaryOv.classList.add('show'); }
  ...
  function startLevel(lvl, isRestart=false, showOverlayAtStart=false){ level = lvl; elLevel.textContent = `Level ${level}`; rng = mulberry32((Date.now() + level*1337) % 2**31); player.x = WORLD.w*0.5 - player.w/2; player.y = 0; player.vx=0; player.vy=0; player.onGround=false; player.coyoteTime=0; player.jumpBuffer=0; levelTime = 0; elLevelTime.textContent = formatTime(0); goldThisLevel = 0; comboClimbThisAir = 0; bestComboThisLevel = 0; if (!isRestart) levelProgressBestY[level] = player.y; camY = -WORLD.h*0.1; let gapMin = clamp(90 + (level-1)*6, 90, 160); let gapMax = clamp(130 + (level-1)*10, 140, 220); let widthMin = clamp(120 - (level-1)*5, 70, 120); let widthMax = clamp(200 - (level-1)*6, 90, 200); let moveChance = clamp(0.08 + (level-1)*0.04, 0.08, 0.6); let moveRangeMin = 30, moveRangeMax = 80; if (kidMode){ gapMin=Math.max(60, gapMin-25); gapMax=Math.max(90, gapMax-35); widthMin=Math.min(170, widthMin+40); widthMax=Math.min(260, widthMax+40); moveChance=Math.max(0.04, moveChance*0.5); moveRangeMin=18; moveRangeMax=44; } const height = clamp(1200 + (level-1)*220, 1200, 3600); currentLevelHeight = height; goalY = -height; platforms = []; platforms.push({ x: WORLD.w*0.5 - 180, y: 40, w: 360, h: 18, type:'static', phase:0 }); let lastY = 40 - randRange([40,60]); while (lastY > goalY + 80){ const w = randRange([widthMin, widthMax]); const x = clamp(randRange([24, WORLD.w - 24 - w]), 24, WORLD.w - 24 - w); const gap = randRange([gapMin, gapMax]); const y = lastY - gap; if (chance(moveChance)){ const range = randRange([moveRangeMin, moveRangeMax]); platforms.push({ x, y, w, h:16, type:'moving', vx:0, range, _origX:x, phase: randRange([0, Math.PI*2]) }); } else { platforms.push({ x, y, w, h:16, type:'static', phase:0 }); } lastY = y; }
    const key = STORE.pb(level); const prev = parseFloat(localStorage.getItem(key)); elPB.textContent = `PB: ${isFinite(prev) ? formatTime(prev) : '--:--.--'}`; updateGoldUI(); refreshGiftButton(); if (showOverlayAtStart){ gameState='overlay'; btnStart.textContent='Start'; showOverlay(`Level ${level}`, 'Tap Start to play', true, isRestart); } else { setRunning(); } }
  function flashStatus(text){ elStatus.textContent=text; elStatus.style.opacity='1'; elStatus.style.transition='none'; requestAnimationFrame(()=>{ elStatus.style.transition='opacity 1.6s ease 0.6s'; elStatus.style.opacity='0'; }); }
  ...
  function buildShop(){ skinGrid.innerHTML = ''; SKINS.forEach(skin=>{ const card = document.createElement('div'); card.className='skin-card'; card.dataset.skin=skin.id; card.classList.add(`rarity-${skin.rarity}`);
      ...
      btnBuy.addEventListener('click', ()=>{ if (gated) return; if (gold >= skin.price){ gold -= skin.price; saveGold(); updateGoldUI(); inventory.push(skin.id); saveInventory(); btnBuy.style.display='none'; btnEquip.style.display=''; btnEquip.disabled=false; SFX.coin(); updateStatsUI(); } });
      btnEquip.addEventListener('click', ()=>{ if (!inventory.includes(skin.id)) return; equippedSkinId = skin.id; saveSkin(); SFX.equip(); [...document.querySelectorAll('.skin-card')].forEach(c=>{ const sid = c.dataset.skin; const eqBtn = c.querySelector('.skin-actions .btn:last-child'); if (eqBtn) eqBtn.disabled = (sid===equippedSkinId); }); });
    }); }
  function drawSkinPreview(cnv, skin, t){ const c2 = cnv.getContext('2d'); const phase = parseFloat(cnv.dataset.phase||'0'); const time = (t||0)/1000 + phase; const bob = Math.sin(time*2.0) * 6; c2.clearRect(0,0,cnv.width, cnv.height); const grd = c2.createLinearGradient(0,0,0,cnv.height); grd.addColorStop(0, 'rgba(15,16,18,0.7)'); grd.addColorStop(1, 'rgba(10,11,13,0.5)'); c2.fillStyle = grd; c2.fillRect(0,0,cnv.width, cnv.height); c2.fillStyle='#242325'; c2.strokeStyle='rgba(255,255,255,0.05)'; c2.lineWidth=1.2; roundRect(c2, 16, 86, 200, 16, 6); c2.fill(); c2.stroke(); const px = 76, py = 56 + bob, w=44, h=56; c2.fillStyle=skin.body; roundRect(c2, px, py, w, h, 10); c2.fill(); neonFill(c2, ()=>{ roundRect(c2, px+8, py+10, w-16, 16, 8); c2.fillStyle=skin.visor; c2.fill(); }, skin.visor, 10); neonFill(c2, ()=>{ roundRect(c2, px + w*0.15, py + h - 12, w*0.7, 6, 3); c2.fillStyle=skin.stripe; c2.fill(); }, skin.stripe, 8); }
  ...
  let audio = null; function ensureAudio(){ if (audio) return; try{ const ctx = new (window.AudioContext || window.webkitAudioContext)(); const master = ctx.createGain(); master.gain.value = muted? 0 : 0.6; master.connect(ctx.destination); audio = { ctx, master }; }catch{} }

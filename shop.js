...
// v5.3.1 HOTFIX shop.js — restores working tabs & buy/equip
(() => {
  'use strict';
  ...
  if(!grid || !tmpl){ console.warn('[shop] Missing base DOM'); return; }
  ...
  function render(type){
    if(type==='stats'){ const sRaw=localStorage.getItem(K.STATS); const s=sRaw?JSON.parse(sRaw):{}; const his=Number(localStorage.getItem('hiscore')||0); const maxL=maxLevel(); grid.innerHTML=`<article class='card'><div>Best Height</div><div>${his}</div></article><article class='card'><div>Max Level</div><div>${maxL}</div></article><article class='card'><div>Total Jumps</div><div>${s.jumps||0}</div></article><article class='card'><div>Coins</div><div>${s.coins||0}</div></article>`; return; }

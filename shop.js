...
(()=>{
  ...
  const grid=document.getElementById('grid'); const tabs=[...document.querySelectorAll('.tab')];
  const tmpl=document.getElementById('item-card'); const walletEl=document.getElementById('wallet');
  const PRICES={common:200};
  const CATALOG={
    themes:[{id:'theme-default',name:'Default',rarity:'common',preview:{platform:'#5e81ac',bg:'#0f1a2b'}}],
    backgrounds:[{id:'bg-default',name:'Deep Night',rarity:'common',preview:{bg1:'#0f1a2b',bg2:'#162238'}}],
    skins:[{id:'skin-default',name:'Blue Buddy',rarity:'common',preview:{player:'#7cd1f9'}}],
    accessories:[{id:'head-cap',name:'Blue Cap',slot:'head',rarity:'common',price:200,minLevel:3,preview:'CAP'}]
  };
  ...
  function render(type){
    if(type==='stats'){ const sRaw=localStorage.getItem(K.STATS); const s=sRaw?JSON.parse(sRaw):{}; const his=Number(localStorage.getItem('hiscore')||0); const maxL=maxLevel(); grid.innerHTML='<article class="card">Best Height: '+his+'</article><article class="card">Max Level: '+maxL+'</article>'; return; }
    const items=CATALOG[type]; const owned=loadOwned(); const eq=loadEquipped(); grid.innerHTML='';
    items.forEach(item=>{ const node=tmpl.content.firstElementChild.cloneNode(true); const prev=node.querySelector('.preview'); const nameEl=node.querySelector('.name'); const rEl=node.querySelector('.rarity'); const btn=node.querySelector('.buy'); nameEl.textContent=item.name; rEl.textContent=item.rarity.toUpperCase();
      if(type==='skins'){ const d=document.createElement('div'); d.style.cssText='width:40px;height:52px;border-radius:6px;background:'+item.preview.player; prev.appendChild(d); }
      else if(type==='themes'){ prev.style.background=item.preview.bg; const p=document.createElement('div'); p.style.cssText='width:70%;height:16px;border-radius:8px;background:'+item.preview.platform; prev.appendChild(p); }
      else if(type==='backgrounds'){ prev.style.background='linear-gradient(180deg,'+(item.preview.bg1||'#0f1a2b')+','+(item.preview.bg2||'#162238')+')'; }
      else if(type==='accessories'){ prev.textContent=item.preview||'HAT'; }

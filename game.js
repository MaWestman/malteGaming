...
(() => {
  ...
  // Celebration vars
  let celebrateT=0, flashT=0, shakeT=0; // seconds
  ...
  window.addEventListener('keydown', e=>{ if(e.code==='Space'){ e.preventDefault(); if(state==='celebrating' && pendingNext){ // allow space to continue
        onContinueNext();
      } else { queueJump(); incStat('jumps',1);} } if(e.code==='ArrowLeft'||e.code==='KeyA') moveLeft=true; if(e.code==='ArrowRight'||e.code==='KeyD') moveRight=true; });
  ...
  function emit(color,x,y,vx,vy,life=0.8,size=3){ parts.push({x,y,vx,vy,life,color,size}); }
  function burst(x,y,color,count=26,spd=340){ for(let i=0;i<count;i++){ const a=Math.random()*Math.PI*2, s=spd*(0.35+Math.random()*0.9); emit(color,x,y,Math.cos(a)*s,Math.sin(a)*s,0.6+Math.random()*0.8,3+Math.random()*2); } }
  ...
  function tryJump(){ if(player.jumpsLeft>0 && state==='playing'){ player.vy=JUMP_V; player.onGround=false; player.jumpsLeft--; burst(player.x+player.w/2, player.y+player.h/2, '#7cd1f9', 14, 260); } }
  ...
  function setGoalForLevel(){
    ...
    goalY=playerStartY - dynamicLevelHeight; // top
    const platY = goalY + 10; const plat = {x:0, y:platY, w:W, h:24}; platforms.push(plat); FINAL_PLAT=plat;
    const gx=Math.floor(W/2 - 18); GOAL={x:gx, y:goalY-60, w:36, h:120, reached:false};
  }
  ...
  function finishLevel(){ // trigger celebration; no auto-advance
    ...
    flashT=0.45; shakeT=0.8; celebrateT=2.0; state='celebrating';
  }
  ...
  function update(dt){
    ...
    if(state==='celebrating'){
      ...
      if(celebrateT<=0 && !shownOverlayAfterWin){ state='waiting'; shownOverlayAfterWin=true; pendingNext=true; showOverlay('Level Complete! Continue ▶'); syncUiVisibility(); }
      return;
    }

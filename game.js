...
(() => {
  ...
  const GRAVITY=2200, JUMP_V=-900, MAX_FALL=1700, MOVE_ACC=2200, MOVE_MAX=360, FRICTION=2000; const ALLOW_DOUBLE=true; let P_W_MIN=90,P_W_MAX=180,P_GAP_Y_MIN=70,P_GAP_Y_MAX=130,P_MARGIN=30; const COIN=18; const MAX_LEVEL=100;  
  ...
  const canvas=document.getElementById('game'); const ctx=canvas.getContext('2d'); function scaleCanvas(){ const dpr=Math.max(1,Math.floor(window.devicePixelRatio||1)); canvas.width=W*dpr; canvas.height=H*dpr; ctx.setTransform(dpr,0,0,dpr,0,0);} scaleCanvas(); window.addEventListener('resize', scaleCanvas,{passive:true});
  ...
  function startLevel(n){ activeLevel=Math.max(1,Math.min(100,n)); applyDifficulty(); resetWorld(); state='waiting'; setOverlay(`Start Level ${activeLevel}`); levelEl && (levelEl.textContent=String(activeLevel)); syncUiVisibility(); incStat('runs',1); }
  function nextLevel(){ const next=Math.min(100, activeLevel+1); const cur=Number(localStorage.getItem(K.MAXLVL)||1); if(next>cur) localStorage.setItem(K.MAXLVL,String(next)); startLevel(next); }
  ...
  function placePlayerOnPlatform(){ if(!platforms.length) return; let p=platforms[0]; for(const pl of platforms){ if(pl.y>p.y) p=pl; } const xCenter=p.x; const right=p.x+p.w-36; player.x=Math.max(p.x,Math.min(right,xCenter)); player.y=p.y-player.h; player.vx=0; player.vy=0; player.onGround=true; player.jumpsLeft=ALLOW_DOUBLE?2:1; }
  ...
  function update(dt){ if(state==='playing') addTime(dt);
    if(player.vx>10) ; else if(player.vx<-10) ;
    if(window._ml){ /* noop */ }
    if(player.vx>0 && !false){} // keep lints quiet
    if(false){}
    if(window._ml2){}
    if(window._ml3){}

    if(window._left || false){}
    if(window._right || false){}

    if(window._tmp){}
    // Controls
    if((window._L=0,0),0); // placeholder no-op

    // Horizontal control
    if((window._u=0,0),0); // no-op lines

    // Move left/right
    // Using MOVE_ACC/FRICTION model
    // (kept identical to previous stable build semantics)

    // We'll re-implement succinctly here
    const MOVE_ACC=2200, MOVE_MAX=360, FRICTION=2000;
    let moveLeft=false, moveRight=false; // we will query keys by listening globals

    // Query keys from stored flags (set by listeners)
    moveLeft = !!window.__left; moveRight = !!window.__right;
    ...
    // Jump queue
    if(window.__jump && state==='playing'){ tryJump(); window.__jump=false; }

    // Integrate
    const GRAVITY=2200, MAX_FALL=1700;
  ...
  function draw(){ const V=visuals(); ctx.fillStyle=V.bg1; ctx.fillRect(0,0,W,H); ctx.fillStyle=V.bg2; ctx.fillRect(0,0,W,H*0.6); ctx.fillStyle=V.para; for(let i=0;i<10;i++){ const w2=160,h2=60; const xx=((i*180)-(performance.now()*0.02)%(W+200))-100; const yy=220+Math.sin(i)*10 - cameraY*0.05; ctx.fillRect(xx,yy,w2,h2);} ctx.fillStyle=V.platform; for(const p of platforms){ ctx.fillRect(p.x, p.y-cameraY, p.w, 20);} for(const c of coins){ if(!c.active) continue; const cx=c.x+c.w/2, cy=c.y+c.h/2 - cameraY, r=c.w/2; ctx.fillStyle='#ffd166'; ctx.beginPath(); ctx.arc(cx,cy,r,0,Math.PI*2); ctx.fill(); ctx.strokeStyle='#e9c46a'; ctx.lineWidth=2; ctx.stroke(); }
    ...
    drawShadow(player.x,player.y,player.w,player.h); const dx=player.x, dy=player.y-cameraY; ctx.save(); ctx.fillStyle='#7cd1f9'; ctx.fillRect(player.x, player.y-cameraY, player.w, player.h); ctx.restore(); if(GOAL){ drawFlag(ctx, GOAL.x, GOAL.y, GOAL.w, GOAL.h); }
  ...
  // Key listeners to set flags used above
  window.addEventListener('keydown', e=>{ if(e.code==='Space'){ e.preventDefault(); window.__jump=true; } if(e.code==='ArrowLeft'||e.code==='KeyA') window.__left=true; if(e.code==='ArrowRight'||e.code==='KeyD') window.__right=true; });
  window.addEventListener('keyup', e=>{ if(e.code==='ArrowLeft'||e.code==='KeyA') window.__left=false; if(e.code==='ArrowRight'||e.code==='KeyD') window.__right=false; });
  const btnJump=document.getElementById('btnJump'); btnJump && btnJump.addEventListener('pointerdown', ()=>{ window.__jump=true; });
  const btnLeft=document.getElementById('btnLeft'), btnRight=document.getElementById('btnRight');
  function hold(btn,flag){ if(!btn) return; const on=()=>window[flag]=true, off=()=>window[flag]=false; btn.addEventListener('pointerdown',on); ['pointerup','pointerleave','pointercancel'].forEach(ev=>btn.addEventListener(ev,off)); }
  hold(btnLeft,'__left'); hold(btnRight,'__right');
  ...
  function updateHUD(){ const scoreEl=document.getElementById('score'); const hiscoreEl=document.getElementById('hiscore'); const levelEl=document.getElementById('level'); if(scoreEl) scoreEl.textContent=String(levelProgress); if(hiscoreEl) hiscoreEl.textContent=String(Number(localStorage.getItem(K.HISCORE)||0)); if(levelEl) levelEl.textContent=String(activeLevel); }
  function syncUiVisibility(){ const mobileBar=document.querySelector('.mobile-bar'); if(!mobileBar) return; const hide=(state==='playing'); mobileBar.classList.toggle('hidden', hide); }
  ...
  function resetWorld(){
    cameraY=0; spawnY=H; parts.length=0; platforms.length=coins.length=enemies.length=0; GOAL=null; FINAL_PLAT=null; touchedFinalPlatform=false;
    Object.assign(player,{ x:W*0.5-18, y:300, vx:0, vy:0, onGround:false, jumpsLeft:2, facing:1, anim:'idle', animTime:0 });
  ...
  let last=performance.now(); resetWorld(); setOverlay('Start Level 1'); syncUiVisibility(); requestAnimationFrame(loop);
})();

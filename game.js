...
(() => {
  ...
  // --- Visual catalogs ---
  // Themes: affect platform color (and fallback background if no background is equipped).
  const THEMES = {
  ...
  // Backgrounds: override bg gradient and parallax color when equipped.
  const BACKGROUNDS = {
  ...
  function getVisuals(){
    // Read fresh each frame so changing theme/background in Shop applies immediately when switching tabs/pages
  ...
  // Skin accent for cosmetic particles (kept)
  ...
  let skinImg=null, skinReady=false; const eq0=loadEquipped(); const ACCENT=skinAccent(eq0 && eq0.skin);
  if(eq0 && eq0.skin){ const img=new Image(); img.onload=()=>{ skinImg=img; skinReady=true; }; img.onerror=()=>{ skinImg=null; skinReady=false; }; img.src=`assets/images/sprites/${eq0.skin}.png`; }
  ...
  function ensureSpawn(){
    ...
    while (spawnY > target){
      ...
      if (w>120 && Math.random()<0.30){
        ...
        if (pick==='slime') spawnSlimeOnPlatform(x,y,w); else spawnRollerOnPlatform(x,y,w);
      }
  ...
  function update(dt){
    ...
    for (const e of enemies){ if(!e.alive) continue; e.anim += dt; if (e.type==='slime' || e.type==='roller'){
        ...
        e.x += e.dir * e.speed * dt; if (e.x<e.left){ e.x=e.left; e.dir=1; } if (e.x>e.right){ e.x=e.right; e.dir=-1; }
        e.t = (e.t||0) + dt * 4; e.y += Math.sin(e.t) * 24 * dt * 8;
      }
      if (aabb(player,e)){
  ...
  function draw(){
    const V = getVisuals();
    // Background using active Theme/Background
    ...
    // Platforms use Theme platform color
    ...
    if(state==='menu') center('Press Start (or tap ⤒). Themes now style platforms & background!', W/2, H/2, 18);
    else if(state==='paused') center('Paused', W/2, H/2, 20);
    else if(state==='gameover'){ center('Game Over — Press Restart', W/2, H/2-10, 22); center(`Height: ${score} · Best: ${hiscore} · +${pointsEarned} pts`, W/2, H/2+18, 14); }
  }

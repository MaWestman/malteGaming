...
(() => {
  'use strict';
  const W = 800, H = 450; const MAX_LEVEL=100;
  ...
  let celebrateT=0, flashT=0, shakeT=0; // timers in seconds

  // Physics & params
  const GRAVITY=2200, JUMP_V=-900, MAX_FALL=1700, MOVE_ACC=2200, MOVE_MAX=360, FRICTION=2000; const COIN=18;  
  ...
  window.addEventListener('keydown', e=>{ if(e.code==='Space'){ e.preventDefault(); if(state==='celebrating' && pendingNext) return onContinueNext(); queueJump(); incStat('jumps',1);} if(e.code==='ArrowLeft'||e.code==='KeyA') moveLeft=true; if(e.code==='ArrowRight'||e.code==='KeyD') moveRight=true; });
  ...
  // Flag sits ON platform
  function setGoalForLevel(){
    dynamicLevelHeight=levelLengthFor(activeLevel);
    goalY=playerStartY - dynamicLevelHeight; // very top of the level
  ...
  function onDeath(){ if(dead) return; dead=true; state='waiting'; const earned=Math.floor(levelProgress/5)+runPoints; addPointsToStats(earned); saveWallet(loadWallet()+earned); showOverlay(`Retry Level ${activeLevel}`); syncUiVisibility(); }
  ...
  function draw(){
    ...
    // Goal flag + celebration rays/glow
    ...
    // Progress bar
    const p = Math.max(0, Math.min(1, levelProgress / dynamicLevelHeight)); drawProgressBar(ctx, 10, 8, W-20, 8, p);

    // Screen flash
    drawFlash();

    // Banner text during celebration or when waiting to continue
  ...
  function finishLevel(){ // trigger celebration; no auto-advance
    try{ sfx.win.currentTime=0; sfx.win.play().catch(()=>{});}catch{}
    const cx=GOAL.x+GOAL.w/2, cy=GOAL.y+10; 
    const palette=['#7de07d','#ffd166','#7cd1f9','#f97098','#c084fc'];
    for(let k=0;k<18;k++){ const col=palette[k%palette.length]; burst(cx, cy, col, 26, 340); }
    flashT=0.45; shakeT=0.8; celebrateT=2.0; state='celebrating'; syncUiVisibility();
  }

  function onContinueNext(){ if(!pendingNext) return; pendingNext=false; hideOverlay(); nextLevel(); }

  function update(dt){
    updateParticles(dt);

    if(state==='celebrating'){
      celebrateT-=dt; flashT-=dt; shakeT-=dt; if(celebrateT<=0 && !shownOverlayAfterWin){ state='waiting'; shownOverlayAfterWin=true; pendingNext=true; showOverlay('Level Complete! Continue ▶'); syncUiVisibility(); }
      return;
    }

    if(state!=='playing') return; // paused/waiting
  ...
  // Overlay button
  levelAction && levelAction.addEventListener('click', ()=>{ if(pendingNext){ onContinueNext(); } else { hideOverlay(); state='playing'; syncUiVisibility(); } });

...
// v5.3.3b game.js — FIX: Retry button truly restarts the round
// - Adds retryPending flag; on Retry click -> resetWorld() and start playing immediately
// - Works for BOTH legacy overlay button (#levelAction) and dock button (#levelActionDock) if present
// - Keeps dock-outside-canvas behavior if you previously used it; otherwise overlay works too.
(() => {
  'use strict';
  const W=800,H=450,MAX_LEVEL=100;
  const K={ HISCORE:'hiscore', WALLET:'wallet', STATS:'stats', MAXLVL:'maxLevelReached' };
  ...
  // Optional dock outside canvas (if a previous build created it)
  const levelActionDock = document.getElementById('levelActionDock');
  const dock = document.getElementById('levelDock');
  ...
  let last=performance.now(); let cameraY=0, spawnY=H, bestY=0, playerStartY=300, runPoints=0;
  ...
  const platforms=[], coins=[], parts=[]; let GOAL=null, FINAL_PLAT=null, goalY=0;
  ...
  function forceHideOverlay(){ if(levelOverlay){ levelOverlay.classList.add('hidden'); levelOverlay.style.display='none'; levelOverlay.setAttribute('aria-hidden','true'); } if(dock){ dock.style.display='none'; } }
  function showOverlay(text){ if(levelAction) levelAction.textContent=text; if(levelOverlay){ levelOverlay.classList.remove('hidden'); levelOverlay.style.display='grid'; levelOverlay.removeAttribute('aria-hidden'); }
    if(levelActionDock){ levelActionDock.textContent=text; if(dock) dock.style.display='block'; }
  }
  function hideOverlay(){ if(levelOverlay){ levelOverlay.classList.add('hidden'); levelOverlay.style.display='none'; levelOverlay.setAttribute('aria-hidden','true'); } if(dock){ dock.style.display='none'; } }
  ...
  function resetWorld(){ cameraY=0; spawnY=H; parts.length=0; platforms.length=coins.length=0; GOAL=null; FINAL_PLAT=null; dead=false; celebrateT=flashT=shakeT=0; pendingNext=false; shownOverlayAfterWin=false;
    Object.assign(player,{ x:W*0.5-18, y:300, vx:0, vy:0, onGround:false, jumpsLeft:2 });
    let y=360; for(let i=0;i<6;i++){ const w=ri(90,180), x=ri(30,W-30-w); platforms.push({x,y,w,h:20}); if(Math.random()<0.6) coins.push({x:x+w*0.5-COIN/2, y:y-32, w:COIN, h:COIN, active:true}); y-=ri(70,130);} spawnY=y; placePlayerOnPlatform(); playerStartY=player.y; bestY=player.y; levelProgress=0; setGoalForLevel(); updateHUD(); }
  ...
  function startLevel(n){ activeLevel=Math.max(1,Math.min(MAX_LEVEL,n)); resetWorld(); state='waiting'; showOverlay(`Start Level ${activeLevel}`); syncUiVisibility(); }
  function retryNow(){ retryPending=false; resetWorld(); hideOverlay(); state='playing'; syncUiVisibility(); }
  function nextLevel(){ const next=Math.min(MAX_LEVEL, activeLevel+1); startLevel(next); }

  function onDeath(){ dead=true; retryPending=true; state='waiting'; showOverlay(`Retry Level ${activeLevel}`); syncUiVisibility(); }

  // Action routing (Start / Retry / Continue) for BOTH overlay and dock
  function handleActionClick(){ if(pendingNext) return onContinueNext(); if(retryPending) return retryNow(); hideOverlay(); state='playing'; syncUiVisibility(); }
  ...
  function celebrate(){ try{ sfx.win.currentTime=0; sfx.win.play().catch(()=>{});}catch{} const cx=GOAL.x+GOAL.w/2, cy=GOAL.y+10; burst(cx,cy,26,340); celebrateT=2.0; state='celebrating'; syncUiVisibility(); }

  // Boot
  function boot(){ startLevel(1); showOverlay('Start Level 1'); syncUiVisibility(); function loop(now){ requestAnimationFrame(loop); const dt=Math.min((now-last)/1000,0.033); last=now; if(state==='playing'||state==='celebrating') update(dt); draw(); } requestAnimationFrame(loop);} boot();
})();

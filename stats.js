...
(() => {
  'use strict';
  const K = { STATS:'stats', HISCORE:'hiscore', MAXLVL:'maxLevelReached', WALLET:'wallet' };
  ...
  const ach = [];
  const maxLvl = Number(localStorage.getItem(K.MAXLVL)||1);
  ...
  achList.innerHTML = ach.map(a=>`<li class="${a.done?'done':''}">${a.done?'✅':'⬜️'} ${a.text}</li>`).join('');
})();
